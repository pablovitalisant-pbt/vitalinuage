...
### Busqueda Flexible de Pacientes (Alineación Completa)
- **Feature Flag:** `patient_search_flexible_v1` (habilitado: `true`) en `config/feature-flags.json`.
- **Endpoints Alineados:** 
  - `GET /api/patients/search` (Búsqueda global)
  - `GET /api/patients` (Lista paginada de la sección Pacientes)
- **Evidencia de Alineación:** Ambos endpoints ahora utilizan `apply_flexible_patient_search()` (`backend/api/patients.py:110-238`).
- **Comportamiento Flexible (Habilitado):** 
  - Busca en 6 campos: `nombre`, `apellido_paterno`, `apellido_materno`, `dni`, `email`, `telefono`.
  - Soporte multi-término: permite encontrar por apellido materno o combinaciones de términos (AND entre términos).
