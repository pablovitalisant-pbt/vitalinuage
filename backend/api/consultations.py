...
async def create_verification(
    ...
    """
    Crea un registro de verificaci贸n para una consulta.
    Reutiliza si ya existe (idempotente).
    
    - **consultation_id**: ID de la consulta
    - **Requiere autenticaci贸n**: Solo el m茅dico due帽o
...
async def send_prescription_email(
    ...
    """
    Env铆a receta por email (as铆ncrono).
    ...
    # 3. Obtener o crear verificaci贸n
    verification = db.query(models.PrescriptionVerification).filter(
        models.PrescriptionVerification.consultation_id == consultation_id
    ).first()
    ...
    # 5. Registrar env韔 (Timestamp)
    verification.email_sent_at = datetime.datetime.utcnow()
    db.commit()

    # 6. A馻dir tarea en background
...
async def get_dispatch_status(
    ...
    # Gracias a la propiedad hibrida a帽adida en models.py, esto es facil,
